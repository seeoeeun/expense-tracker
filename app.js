...
// 편집용 상태
state.editing = null; // { id, name, amount, category, date, memo } or null

// 모달 카테고리 토글 유틸
function selectModalCategory(val) {
  document.querySelectorAll("#eCategoryGroup .circle").forEach((b) => {
...
firebase.auth().onAuthStateChanged((user) => {
  ...
  $("#userLabel").textContent = user
    ? user.displayName || user.email || "로그인 됨"
    : "";
...
function bindDataListeners() {
  // cleanup
  if (state.unsubExpenses) {
    state.unsubExpenses();
    state.unsubExpenses = null;
  }
  if (state.unsubRecurring) {
    state.unsubRecurring();
    state.unsubRecurring = null;
  }
  state.expenses = [];
  state.recurring = [];
...
$("#add").addEventListener("click", async () => {
  ...
  if (!["필수", "투자", "소비"].includes(category))
    return alert("카테고리 오류");
  ...
  if (mk === monthKey(state.year, state.month)) {
    ...
    state.expenses = [temp, ...state.expenses].sort(
...
$("#rAdd").addEventListener("click", async () => {
  ...
  const startMonth = start
    ? parseInt(start.split("-")[1], 10) - 1
    : new Date().getMonth();
  const startYM =
    start || `${startYear}-${String(startMonth + 1).padStart(2, "0")}`;

  await db.collection("users").doc(state.user.uid).collection("recurring").add({
    name,
    amount,
    category,
    day,
...
function setActiveTab(tab) {
  ...
  document.querySelectorAll(".tablink").forEach((btn) => {
    ...
    btn.setAttribute(
      "aria-current",
      btn.dataset.tab === tab ? "page" : "false"
    );
  });
...
// 해당 월의 말일
function lastDateOf(year, month) {
...
function renderCalendar() {
  ...
  for (let day = 1; day <= lastDate; day++) {
    ...
    cell.className =
      "cell day" + (state.selectedDate === dstr ? " selected" : "");
...
function renderList() {
  ...
  const list = [...listBase, ...recSumToday].sort(
  ...
  for (const e of list) {
    ...
    if (!e._recurring) {
      ...
      delBtn.className = "tdelete"; // 또는 rdelete 등 상황에 따라 동일하게 유지
      const delIcon = document.createElement("img");
      delIcon.src = "https://i.ibb.co/8LXbPYst/Frame-1-1.png";
      delIcon.alt = "삭제";
      delBtn.appendChild(delIcon);
      delBtn.addEventListener("click", (ev) => {
        ev.stopPropagation();
        onDelete(e.id); // 반복 항목일 경우 deleteRecurring(r.id)로 맞게 교체
      });
      row.appendChild(delBtn);
    }
    box.appendChild(row);
    row.addEventListener("click", () => openEditSheet(e));
  }
}

function openEditSheet(expense) {
  // 반복에서 유도된 가상 항목이면 읽기 전용
  const isDerived = !!expense._recurring;
  $("#eHelp").style.display = isDerived ? "" : "none";
  $("#eSave").disabled = isDerived;
  $("#eDelete").disabled = isDerived;

  // 값 바인딩
  $("#eId").value = expense.id || "";
  $("#eName").value = expense.name || "";
  $("#eAmount").value = Number(expense.amount || 0);
  $("#eDate").value = expense.date || todayStr();
  $("#eMemo").value = expense.memo || "";
  selectModalCategory(expense.category || "필수");
...
// 모달 내 카테고리 버튼 토글
document.querySelectorAll("#eCategoryGroup .circle").forEach((btn) => {
  btn.addEventListener("click", () => selectModalCategory(btn.dataset.value));
});

// 저장
$("#editForm").addEventListener("submit", async (e) => {
  e.preventDefault();
  if (!state.user) return alert("먼저 로그인해줘");
  ...
  const catBtn = document.querySelector("#eCategoryGroup .circle.is-selected");
  const category = catBtn ? catBtn.dataset.value : "필수";

  // 반복 유도 가상 아이템 보호
  if (!id || String(id).startsWith("rec-")) return;
  if (!name) return alert("이름을 입력해줘");
  if (!isFinite(amount) || amount <= 0) return alert("금액 확인해줘");
  ...
  try {
    await db
      ...
      .collection("expenses")
      .doc(id)
      .update({ name, amount, category, date, monthKey: mk, memo });
  } catch (err) {
...
function renderRecurring() {
  ...
  const list = [...(state.recurring || [])].sort(
    (a, b) => (b.amount || 0) - (a.amount || 0)
  );
  ...
  for (const r of list) {
    ...
    delBtn.addEventListener("click", (ev) => {
      ev.stopPropagation();
      deleteRecurring(r.id); // 반복 항목일 경우 deleteRecurring(r.id)로 맞게 교체
    });
...
function renderSums() {
  ...
  for (const e of state.expenses) {
    if (e.monthKey === mk) sumCat[e.category] += e.amount;
  }
  for (const r of state.recurring) {
    if (!r.active) continue;
    if (
      r.startYear > state.year ||
      (r.startYear === state.year && r.startMonth > state.month)
    )
      continue;
...
function renderTodayList() {
  ...
  for (const e of list) {
    ...
    row.append(dot, name, amt, delBtn);
...
function renderRepeat() {
  console.log("renderRepeat called");
  const box = document.querySelector("#rList");
  console.log("rList found?", !!box);
  if (!box) return;

  // 반복으로 저장해둔 데이터가 어디에 있는지에 따라 이 부분은 맞춰야 함.
  // 아까 네 코드 흐름 보면 반복 추가 버튼에서 state.repeat 같은 데에 push 하고 있었으니까 그걸 그대로 쓴다.
  const list = state && state.repeat ? state.repeat : [];
  console.log("repeat items count:", list.length);
  ...
  if (list.length === 0) {
    const empty = document.createElement("div");
    empty.className = "empty";
    empty.textContent = "등록된 반복 내역이 없습니다.";
    box.appendChild(empty);
    return;
  }
  ...
  list.forEach((e) => {
    ...
    const amt = document.createElement("div");
    amt.className = "ramt";
    // 네가 쓰는 금액 포맷터가 있으면 그걸 쓰고, 없으면 그냥 원시값
    if (typeof fmtKRW === "function") {
      amt.textContent = fmtKRW(e.amount);
    } else {
    ...
    // 여기서 today랑 똑같이 편집 모달 열기
    row.addEventListener("click", () => {
      // 네가 반복용으로 rec- 같은 id를 쓰는 구조라면 여기서 막아주면 되고,
      // 아니라면 바로 openEditSheet(e)만 호출해도 됨
      if (String(e.id || "").startsWith("rec-")) {
        alert("이 반복 예약 항목은 직접 수정할 수 없습니다.");
        return;
      }
      openEditSheet(e);
    });
...
const first = document.querySelector(
  '#rCategoryGroup .cat-btn[data-value="필수"]'
);
...
document.querySelectorAll("#rCategoryGroup .cat-btn").forEach((btn) => {
  btn.addEventListener("click", () => {
    document
      .querySelectorAll("#rCategoryGroup .cat-btn")
...
document.addEventListener("DOMContentLoaded", () => {
  document.querySelectorAll("nav button").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      if (tab === "record") renderList();
      if (tab === "repeat") renderRepeat();
    });
  });

  // 앱 시작 시 Today 탭을 한 번 렌더링
  renderTodayList();
  renderRepeat();
});
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  buttons.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      document.body.className = "tab-" + tab;
      if (tab === "record") {
        // 'Today' 탭은 data-tab="record" 를 사용하므로 오늘 리스트를 렌더링
        renderTodayList();
      }
      if (tab === "calendar") {
        renderCalendar();
      ...
      if (tab === "recurring") {
        // 반복 탭(data-tab="recurring")에서 반복 리스트를 렌더링
        renderRepeat();
      }
      // record 전용 리스트가 따로 필요하면 아래 함수도 호출하세요
      // if (tab === 'record') renderList();
    });
  });
});
